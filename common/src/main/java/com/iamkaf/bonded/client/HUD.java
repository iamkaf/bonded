...
public class HUD {
    ...
    public static void onRenderHud(GuiGraphicsExtractor guiGraphics, DeltaTracker deltaTracker) {
        ...
        boolean hasMenuOpen = mc.gui.screen() != null;
     ...
     */
    private static void renderRepairBenchInfo(GuiGraphicsExtractor guiGraphics, Font textRenderer, int x, int y, LocalPlayer player) {
     ...
     * @param player       The local player.
     */
    private static void renderToolBenchInfo(GuiGraphicsExtractor guiGraphics, Font textRenderer, int x, int y, LocalPlayer player) {
        ItemStack stack = player.getMainHandItem();
        GearTypeLeveler leveler = Bonded.GEAR.getLeveler(stack);
        if (leveler == null) return;
        ...
        TooltipRenderUtil.extractTooltipBackground(guiGraphics, x, y, 20, 20, null);
        guiGraphics.item(upgradeIngredient, x + 2, y + 2);
        TooltipRenderUtil.extractTooltipBackground(guiGraphics, x + 24 + 4, y, 20, 20, null);
        guiGraphics.item(upgrade.getDefaultInstance(), x + 24 + 2 + 4, y + 2);
     ...
     * @param y       The y-coordinate for rendering.
     */
    private static void text(GuiGraphicsExtractor context, Font font, Component message, int x, int y) {
        ClientFunctions.renderText(context, font, message, x, y, WHITE);
    }
     ...
     */
    private static void renderTooltip(GuiGraphicsExtractor guiGraphics, ItemStack stack, int x, int y) {
        ClientFunctions.renderTooltip(guiGraphics, stack, x, y);
    }
}
